...
public class Handler {
   ...
   public void showList() throws IOException { //전체 List 보여주는 함수  
      for(int i=0; i<arr.length; i++) {
         if(arr[i] != null ) {
            bw.write("-------------------------------");
            bw.write("이름 : " + arr[i].getName());
            bw.write("나이 : " + arr[i].getAge());
            bw.write("-------------------------------" + "\n");
	...
	public void deletData(String name) throws IOException { // 이름으로 데이터 삭제시작하는 곳 (오버로딩)
		...
		for(int i=0; i<arr.length;i++ ) {
			if(arr[i] != null) {
				...
				bw.write("자료가 없어서 삭제하지 못했습니다.");
			}
			
		}
		bw.write("몇번을 삭제하겠습니까?  ");	// 목록중에 몇번 살제할지 입력받기
		int num = Integer.parseInt(br.readLine());
		cnt = num;
		if(num == cnt) {
			bw.write("정말 삭제하시겠습니까? yes : 1 / no : 2  ");
